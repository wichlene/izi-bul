...
import { Shield, Users, MapPin, Trophy, CheckCircle, Clock, TrendingUp } from 'lucide-react';
...
export default async function AdminPage() {
  ...
  const { data: recentUsers } = await supabase
    .from('profiles')
    .select('id, username, full_name, total_points, total_finds, created_at, is_premium')
  ...
  const { data: recentQuests } = await supabase
    .from('quests')
    .select('id, title, category_id, total_attempts, total_solved, is_active, is_featured, created_at')
    .order('created_at', { ascending: false })
    .limit(10);

  const stats = [
    { icon: <Users size={20} />, val: usersRes.count || 0, label: 'Toplam Kullanıcı', color: '#a855f7' },
    { icon: <MapPin size={20} />, val: questsRes.count || 0, label: 'Toplam Görev', color: '#ff6b2b' },
    { icon: <Trophy size={20} />, val: subsRes.count || 0, label: 'Toplam Deneme', color: '#22c55e' },
            ...
            <div className="px-5 py-4 border-b border-white/5 flex items-center justify-between">
              <h2 className="font-bold text-white flex items-center gap-2">
                ...
                {(pendingRes.count || 0) > 0 && (
                  <span className="text-xs px-2 py-0.5 rounded-full font-bold" style={{ background: 'rgba(234,179,8,0.2)', color: '#eab308' }}>
                    {pendingRes.count}
                  </span>
                )}
              </h2>
              ...
              {!pendingSubmissions?.length ? (
                ...
                pendingSubmissions.map((s) => (
                  <div key={s.id} className="p-4 flex items-center gap-3">
                    {s.photo_url && (
                      <img src={s.photo_url} className="w-14 h-14 rounded-xl object-cover flex-shrink-0" alt="" />
                    )}
              ...
              {recentQuests?.map((q) => (
                <div key={q.id} className="px-5 py-3 flex items-center gap-3">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-white/30 text-xs">{q.total_attempts} deneme · {q.total_solved} çözüm</div>
                  </div>
                  <div className="flex items-center gap-2">
                    {q.is_featured && <span className="text-xs text-yellow-400">⭐</span>}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-white/5">
                  {['Kullanıcı', 'Puan', 'Görev', 'Premium', 'Kayıt'].map((h) => (
                    ...
                    <td className="px-5 py-3">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          <div className="text-white/30 text-xs">{u.full_name}</div>
                        </div>
                      </div>
                    </td>
                    <td className="px-5 py-3 text-orange-400 font-bold">{u.total_points}</td>
                    <td className="px-5 py-3 text-white/60">{u.total_finds}</td>
                    <td className="px-5 py-3">
                      {u.is_premium ? (
                        <span className="text-xs px-2 py-0.5 rounded-full" style={{ background: 'rgba(168,85,247,0.2)', color: '#a855f7' }}>Premium</span>
                      ) : (
...
function ApproveButton({ submissionId }: { submissionId: string }) {
  return (
    <form action={`/api/admin/approve`} method="POST">
      ...
      <button type="submit" className="flex items-center gap-1 text-xs px-3 py-1.5 rounded-lg font-semibold transition-colors" style={{ background: 'rgba(34,197,94,0.2)', color: '#22c55e' }}>
        <CheckCircle size={12} />
        Onayla
      </button>
    </form>
...
function FeatureToggle({ questId, isFeatured }: { questId: string; isFeatured: boolean }) {
  return (
    <form action="/api/admin/feature" method="POST">
      ...
      <button type="submit" className="text-xs px-2 py-1 rounded-lg transition-colors" style={isFeatured ? { background: 'rgba(255,215,0,0.2)', color: '#ffd700' } : { background: 'rgba(255,255,255,0.05)', color: 'rgba(255,255,255,0.3)' }}>
