...
export default async function FollowingPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data } = await admin
    .from('friendships')
    .select('following:profiles!friend_id(id, username, full_name, avatar_url, total_finds)')
    .eq('user_id', id)
    .limit(100);

  const following = (data || []).map((r) => {
    const f = (r as unknown as { following: unknown }).following;
  ...
  return (
    <AppShell>
      <div className="sticky top-0 z-10 flex items-center gap-3 px-4 h-14" style={{ background: '#fff', borderBottom: '1px solid #eff3f4' }}>
        ...
        <div>
          <p className="font-black text-sm" style={{ color: '#0f1419' }}>Takip</p>
          <p className="text-xs" style={{ color: '#536471' }}>@{profile.username} · {following.length}</p>
        </div>
      </div>
      <div>
        {following.length === 0 ? (
          <div className="p-16 text-center text-sm" style={{ color: '#536471' }}>Henüz kimseyi takip etmiyor.</div>
        ) : following.map((u) => (
